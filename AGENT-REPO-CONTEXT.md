...
3. Find project UUID `BOGUS_UUID_REPLACE_ME`.
4. Verify that its current identifier is `BOGUS_TAG_REPLACE_ME` and record its current name.
...
7. Retain both its human-readable identifier, such as `BOGUS_TAG_REPLACE_ME-30`, and its full work-item UUID before planning or mutation.
